...
const logger = createClientLogger('passport')
...
passport.use(
  new LocalStrategy(function (username, password, cb) {
    logger.verbose('GetUserByUserName: ', username)
    userDao
...
passport.serializeUser(function (user: any, cb) {
  logger.verbose('serializeUser: ', user)
  cb(null, user._id)
})

passport.deserializeUser(function (id: string, cb) {
  logger.verbose('deserializeUserID: ', id)
  userDao
    .getUserByUserId(id)
    .then((user) => {
      logger.verbose('deserializeUser user: ', user)
      cb(null, user)
    })
    .catch((err) => {
      if (err) {
